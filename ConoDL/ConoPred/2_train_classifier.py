...
from models.data_processing.utils import Dataset_classifier
...
device = 'cuda:0'
##data
with open('./dataset/train_classifier.json') as f:
    data = json.load(f)
train_dataset = Dataset_classifier(data['train'], 75)
test_dataset = Dataset_classifier(data['test'], 75)
...
model_class = Model_classifier(device, './output/wae/model_epoch_493.pt').to(device)
...
for epoch in range(1, 100+1):
    ...
    if epoch % 5 == 0:
        torch.save(model_class.state_dict(), './output/classifier/' + 'classifier_%s.tar'%str(epoch))